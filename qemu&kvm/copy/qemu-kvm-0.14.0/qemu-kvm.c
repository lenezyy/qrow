...
//add by zyy
struct kvm_run *handle_pio_run = NULL;
//add by zyy
long read_file_size = 0;
...
int kvm_run(CPUState *env)
{
    ...
    int fd = env->kvm_fd;
    //add by zyy
    handle_pio_run = env->kvm_run;
    handle_pio_run->pio_rr.sector_num = -1;
    handle_pio_run->pio_rr.nb_sectors = 0;
    //add by zyy
  again:
...
// add by cuilei
	...
	else if(replay_vm && ! is_replay_initialized)
	{
		...
		for(i=0; i<KVM_RR_RING_BUF_SIZE; i++)
		{
			...
			read_file_size += KVM_MAX_LOG_SIZE;
			printf("Read file size during replay 1, %u \n", read_file_size);
			if(r < 0)
			{
				fprintf(stderr, "Could not read from logfile \n");
				abort();
			}
			if(r)
			{
			...
			{
				// log file exhausted , do more the ptr and don't increment the available buffers
				break;
			}
		}
...
#endif
	// kvm rr
	// if kvm exit occured because of file op 
	// request on ring buffers
	// then flush the buffers and check if anything has to be handled 
	// or not
	if(run->exit_reason & KVM_EXIT_FILE_OP_REQ_MASK)
	{
		if(kvm_recording())
		{
			while(run->ring_buf_user_ptr != run->ring_buf_kvm_ptr)
			{
				r = write(log_file_fd, run->ring_buffers[run->ring_buf_user_ptr], KVM_MAX_LOG_SIZE);
			//	printf("r:%d\n",r);
				if(r<0)
				{
					fprintf(stderr,"Couldn't write to logfile\n");
					abort();
				}
				run->ring_buf_user_ptr = (run->ring_buf_user_ptr + 1) % KVM_RR_RING_BUF_SIZE;
				run->used_buffers--;
		...
		{
			//printf("ring_buf_user_ptr:%d ring_buf_kvm_ptr:%d used_buffers:%d\n",run->ring_buf_user_ptr,run->ring_buf_kvm_ptr,run->used_buffers);
			while(run->ring_buf_user_ptr != run->ring_buf_kvm_ptr)
			{
				...
				//printf("r:%d\n",r);
				//read_file_size += KVM_MAX_LOG_SIZE;
				//printf("Read file size during replay 2, %u \n", read_file_size);
				if(r<0)
				{
		...
		run->exit_reason = run->exit_reason & ~KVM_EXIT_FILE_OP_REQ_MASK;
		// is it only buff request 
		if(run->invalid_exit_reason)
		{
			r=0;
			run->invalid_exit_reason = 0;
			goto again;
		}
	}
	// end kvm rr
