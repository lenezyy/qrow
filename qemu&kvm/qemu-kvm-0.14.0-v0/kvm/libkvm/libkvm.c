...
// kvm rr
int kvm_start_record(kvm_context_t kvm, struct kvm_record_arg *args)
{
	return ioctl(kvm->vm_fd, KVM_ENABLE_RR, args);
}
int kvm_stop_record(kvm_context_t kvm)
{
	return ioctl(kvm->vm_fd, KVM_DISABLE_RR,0);
}
int kvm_start_replay(kvm_context_t kvm, struct kvm_record_arg *args)
{
        return ioctl(kvm->vm_fd, KVM_ENABLE_RPLY, args);
}
// end kvm rr
...
again:
	...
	if (1) {
		...
		case KVM_EXIT_IO:
			// kvm rr 
			if(run->is_replaying && !run->io.direction)
			{
				break;
			}
			// end kvm rr
		...
		case KVM_EXIT_MMIO:
			// kvm rr
			if(run->is_replaying && !run->mmio.is_write)
			{
				break;
			}
			// end kvm rr
