...
int Qrow_create(char *img_file, uint64_t disk_size, uint32_t block_size)
{
	...
	char *magic = "qrow";
	char *map_path = "file/qrow2.map";//先写死，功能实现后再利用irow_generate_filename()优化 
	/*
...
{	
	...
	fd = open(img_file, O_RDWR|O_BINARY);//打开磁盘文件
	...
	{
		is_record = 1;
		//log_file_fd = open(log_file, O_RDWR|O_CREAT|O_TRUNC,0333);
...
int Qrow_close(QrowState *qrow_state)
{
		...
		qrow->block_offset = qrow_state->block_offset;    //这里报错了    
		lseek(qrow_state->img_file, 0, SEEK_SET);
...
*/
int read_log(int img_file_fd, int log_file_fd, uint8_t *tmp_data,FILE * outFile) //返回log文件中记录读取数据存储在物理磁盘对应扇区的数组 
{
	uint8_t  log_type;
	...
	fprintf(outFile,"read:nb_sectors:%d ",nb_sectors);
	...
	for (int i = 0; i < nb_sectors; i++) 
	{
		...
		fprintf(outFile,"offset_sector:%d   ",offset_sector);
	}
	if(rr_io)
	{
		free(rr_io);
...
int main()
{
	...
	FILE * outFile;
	if((outFile = fopen ("file/log_record33.txt", "wt"))==NULL)
    {
        printf("cant open the outfile");
        exit(0);
	}
		
	if(Qrow_open(qrow_state, img_file, 1, log_file))
	{
		printf("open ok\n");
	}
	else
	...
	uint8_t str[5000];
	int fout;
	fout = open("file/result_play3", O_RDWR|O_CREAT|O_TRUNC|O_BINARY); 
	
	for(int i = 0 ; i < 45; i++)  //log文件中记录的读操作次数 
	{
		memset(str,'\0',sizeof(str));	
		int num = read_log(qrow_state->img_file, qrow_state->log_file_fd, str,outFile);
		write(fout, str, num*512);
