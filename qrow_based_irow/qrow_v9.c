...
//added by zyy 
SectorCache *sector_cache = NULL;//用来缓存上一次读取的最大八个扇区的数据
//added end 
...
static int qrow_open_meta_file(BlockDriverState *bs, BDRVQrowState *bqrows,int flags) {
	...
	*/
	...
	if(sector_cache == NULL) {
		sector_cache = qemu_mallocz(sizeof(SectorCache));
		if(sector_cache != NULL) {
			sector_cache->cache = qemu_memalign(512, 4096);
			if(sector_cache->cache != NULL)
				memset(sector_cache->cache, 0, 4096);
			else {
				fprintf(stderr, "Failed to create sector_cache\n");
				ret = -1;
				goto end;
			}
			sector_cache->len = 0;
			sector_cache->sector_map={0};
		} else {
			fprintf(stderr, "Failed to create sector_cache \n");
			ret = -1;
			goto end;
...
int64_t binarySearch(int64_t a[], int n, int64_t key)
{
...
static int qrow_read(BlockDriverState *bs, int64_t sector_num, uint8_t *buf, int nb_sectors) {
	...
	int k = 0; 
	int count = 0;
	readState_ptr = readState + (nb_sectors - 1) ;
	if (sector_cache->len ==  0 || readState->sector_num > sector_cache->sector_map[sector_cache->len-1] || readState_ptr->sector_num < sector_cache->sector_map[0])
	{
		for (; k < nb_sectors; k++) 
		{
			readState_ptr = readState + k;
			if(bdrv_read(s->qrow_data_file, readState_ptr->sector_num, buf+readState_ptr->index*BDRV_SECTOR_SIZE, 1) < 0) {
	...
	else
	{
		...
		{
			readState_ptr = readState + k;
			if (count < sector_cache->len)
			{
				int64_t index = binarySearch(sector_cache->map, sector_cache->len, readState_ptr->sector_num );
				if (index != -1 )
				{
					memcpy(buf+readState_ptr->index*BDRV_SECTOR_SIZE, sector_cache->cache + index*BDRV_SECTOR_SIZE, BDRV_SECTOR_SIZE);
					count++;
				}
				else
				{
	...
	//把该次读取的数据按照bubbleSort排序后的结果填充到sector_cache中
	memset(sector_cache->cache, 0, 4096);
	sector_cache->map={0};
	int y = 0;
	if (nb_sectors >= 8) 
	{
		for(; y < 8; y++)
		{
				...
				readState_ptr = readState + y;
				memcpy(sector_cache->cache+y*BDRV_SECTOR_SIZE, buf+readState_ptr->index*BDRV_SECTOR_SIZE,  BDRV_SECTOR_SIZE);
				sector_cache->sector_map[y] = readState_ptr->sector_num;	
		}
		sector_cache->len = 8;
	}
	else
...
static int qrow_write(BlockDriverState *bs, int64_t sector_num, const uint8_t *buf, int nb_sectors) {
	...
	if(bdrv_write(s->qrow_data_file,s->sector_offset, buf, nb_sectors) < 0) {
...
static void qrow_aio_readv_cb(void *opaque, int ret) {
	...
	for (; k < acb->nb_sectors; k++) 
		{
			readState_ptr = readState + k;
			if(bdrv_read(bqrows->qrow_data_file, readState_ptr->sector_num, buf+readState_ptr->index*BDRV_SECTOR_SIZE, 1) < 0) {
					fprintf (stderr, "Failed to read the  data from %s\n", bqrows->data_file);
					ret= -1;
					goto end;
				}
